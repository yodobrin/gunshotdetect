...
my_path = "/Users/yoavdobrin/Downloads"
labels = np.load(my_path+"/augmented_labels.npy")
...
def create_gunshot_wav_file(microphone_data, index, timestamp):
    temp_fn = my_path+"/Gunshot_Sound_Sample #"+ str(index) + " ("+ str(timestamp) + ").wav"
    
    sf.write(temp_fn, microphone_data, 22050, 'PCM_24')
    # librosa.output.write_wav(my_path+"/Gunshot_Sound_Sample #"
    #                         + str(index) + " ("
...
interpreter_1 = tf.lite.Interpreter(model_path = my_path+"/1D.tflite")
...
interpreter_2 = tf.lite.Interpreter(model_path = my_path+"/128_x_64_2D.tflite")
...
interpreter_3 = tf.lite.Interpreter(model_path = my_path+"/128_x_128_2D.tflite")
